...
3. Identify the exact artifact: GitHub, Hugging Face, Zenodo, Zenn, Reddit, X/Twitter, email, or release tag. (arXiv is not part of the 2026-06-07 launch; if a future release adds arXiv, see the arXiv section below.)
...
The paper v5 release uses **three separate HuggingFace repositories** (one per quantization tier). The rollback can be applied per-repo (if only one quant has the issue) or to all three at once.
...
# Re-private all three at once (broad rollback):
foreach ($repo in $HFRepos) {
    Write-Host "Re-privating $repo ..."
    hf repo visibility $repo --private
}

# Or re-private just one (targeted rollback, e.g. only Q2_K is wrong):
hf repo visibility "morphicode-jp/gemma-4-31B-it-L25L26x1.5-Q2_K" --private
```

Then inspect each affected repo and repair:

```powershell
foreach ($repo in $HFRepos) {
    Start-Process "https://huggingface.co/$repo/tree/main"
}

# Re-push the correct README/license per repo (per-quant README varies):
$ReleaseDir = "publish/release_v1"
hf upload "morphicode-jp/gemma-4-31B-it-L25L26x1.5-IQ1_M"  "$ReleaseDir\HF_README_Q1.md"   README.md --commit-message "fix README after rollback"
hf upload "morphicode-jp/gemma-4-31B-it-L25L26x1.5-Q2_K"   "$ReleaseDir\HF_README_Q2.md"   README.md --commit-message "fix README after rollback"
hf upload "morphicode-jp/gemma-4-31B-it-L25L26x1.5-Q4_K_M" "$ReleaseDir\HF_README_Q4.md"   README.md --commit-message "fix README after rollback"

foreach ($repo in $HFRepos) {
    hf upload $repo "$ReleaseDir\LICENSE-WEIGHTS" LICENSE-WEIGHTS --commit-message "fix weights license notice"
    hf upload $repo "$ReleaseDir\LICENSE"         LICENSE         --commit-message "fix Apache 2.0 license text"
}
```

If a wrong file was uploaded to one specific repo, delete only the wrong path:

```powershell
hf repo delete-files "<repo>" "<wrong-file-name>"
```

Republish (set `--no-private`) only after the private inspection gate in `HF_PRIVATE_PUBLIC_UPLOAD_CHECKLIST.md` passes again for the affected repo(s).
...
## arXiv (NOT in 2026-06-07 launch — future contingency only)

This section is retained for the future case where an arXiv preprint is added (after endorsement is obtained). It is not relevant to the 2026-06-07 launch.
