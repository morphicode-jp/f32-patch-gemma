"""K² meta-optimization: use hagen to tune hagen's own dispatch params.
...
全論 Ch18 K² pattern の具現化:
  K1 = BBOB 問題の解
  K2 = K1 の最適化手法 (hagen の dispatch params)

hagen を使って hagen の dispatch params を最適化 — 自己参照 / fixed point of
...
Eval fn: hagen on BBOB 4 問題 × 2 seeds → sum of -|gap| → maximize
...
def _cfg_from_params(params):
    cfg = dict(zip(HAGEN_PARAM_NAMES, params))
    cfg["random_restart_count"] = int(round(cfg["random_restart_count"]))
    return cfg


def _eval_hagen_config(params, seeds=(42, 43), meta_run_budget=25, verbose=False):
    """Meta eval: score hagen's performance with given dispatch params across problems."""
    ...
    for prob_name, fn, ranges, optimum in PROBLEMS:
        for seed in seeds:
            try:
                r = hagen(fn, ranges, time_budget=meta_run_budget,
...
def main():
    print("=" * 70)
    print("K² meta-optimization of hagen dispatch params")
    print("=" * 70)
    print(f"Param ranges: {dict(zip(HAGEN_PARAM_NAMES, HAGEN_PARAM_RANGES))}")

    # Baseline: current hardcoded defaults
    t0 = time.time()
    print(f"\n[Baseline] current defaults ({dict(zip(HAGEN_PARAM_NAMES, HARDCODE_DEFAULTS))})")
    print("  measuring...", flush=True)
    baseline_score = _eval_hagen_config(HARDCODE_DEFAULTS,
                                         seeds=(42, 43),
                                         meta_run_budget=25)
    baseline_wall = time.time() - t0
    print(f"  baseline meta-score = {baseline_score:.4f}  (wall {baseline_wall:.0f}s for 4×2 runs)")

    # Meta-optimize: use hagen on itself
    print(f"\n[K² meta-opt] budget = 30 min, calling hagen on _eval_hagen_config")
    ...
    def meta_eval_wrapped(p_list):
        return _eval_hagen_config(p_list, seeds=(42, 43), meta_run_budget=25)

    t_meta = time.time()
    r_meta = hagen(
        eval_fn=meta_eval_wrapped,
        param_ranges=HAGEN_PARAM_RANGES,
        param_names=HAGEN_PARAM_NAMES,
    ...
    verify_score = _eval_hagen_config(r_meta["best_params"],
                                       seeds=(42, 43),
                                       meta_run_budget=25)
    ...
    for prob_name, fn, ranges, optimum in PROBLEMS:
        gaps = []
        for seed in (42, 43):
            r = hagen(fn, ranges, time_budget=25,
    ...
    with open("benchmark_hagen_meta.json", "w") as f:
    ...
    print(f"  recommendation: {'UPDATE hagen_params.json' if verify_score > baseline_score else 'keep defaults'}")
