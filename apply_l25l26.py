"""apply_l25l26.py — Apply L25+L26 ×1.5 (8-byte F32 patch) to any Gemma 4 31B GGUF.

Usage:
  python apply_l25l26.py <path_to_gemma4_31b.gguf> [--restore]

What it does:
  - Reads F32 layer_output_scale tensors from layers 25 and 26
  - Multiplies each by 1.5
  - Writes back in-place (creates .backup of original)
  - 8 bytes total modification (2 layers × 4 bytes F32)
  - --restore: undo, restore from .backup

Quant support: All Gemma 4 31B quantizations (IQ1_M / Q2_K / Q4_K_M / Q5_K_M / Q8_0 etc.)
- F32 layer_output_scale is at the same offset across all quantizations.

This is the simpler sibling of basin B (44-byte). Empirically:
- Wins on Q1/Q2 GSM8k (Q1: 24% → 60% (+36pt))
- Comparable to basin B on HellaSwag across all quants
- Discovered manually as a 2-layer baseline

License: Apache 2.0
Author: Akito Hirai (@morphicode_jp)
Repo: https://github.com/morphicode-jp/f32-patch-gemma
"""
import argparse
import hashlib
import os
...
# L25+L26 ×1.5 (2 layers, 8 bytes total)
L25L26_PATCH = {
    25: 1.5,
    26: 1.5,
}
...
def find_layer_output_scale_offsets(gguf_path: str) -> dict:
    """Read GGUF and return dict of {layer_idx: {offset, n_elements}} for layer_output_scale."""
    try:
...
def apply_patch(gguf_path: str, dry_run: bool = False) -> None:
    ...
    print(f"[done] Applied L25+L26 x1.5 patch to {len(plan)} layers.")
    ...
    print(f"[next] To restore original: python apply_l25l26.py {gguf_path} --restore")
...
def main():
    parser = argparse.ArgumentParser(
        description="Apply L25+L26 x1.5 (8-byte F32 patch) to Gemma 4 31B GGUF.",
    )
