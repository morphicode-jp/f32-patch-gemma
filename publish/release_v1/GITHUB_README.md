...
| Qwen 3.6 27B | hybrid (1:3 full:SSM) | +2.5pt |
| Phi-4 14B BF16 | pure dense | null |
| Llama 3.1 / Mistral 7B | pure dense | null |

→ The F32 patch effect is concentrated on hybrid LLMs with rare full-attention layers. Pure dense transformers do not benefit. See the paper (Zenodo) for the mechanistic discussion.
...
This work was developed independently by an oil pump engineer in Japan on nights and weekends. The original 8-byte patch was discovered by an autonomous optimization engine called `morphi` (12-specialist parallel search), which will be released separately. Earlier exploration of a wider 44-byte patch (basin B, paper v3 candidate) showed comparable benchmark gains but has not been validated under interactive conditions and is intentionally not part of this release; see `_archive/basin_b_v1/` in the historical artifacts for that line.

## Releases planned

- **v1.0** (paper v1, 2026-05-27): 8-byte L25+L26 patch + 3 HF GGUFs (Q1/Q2/Q4)
- **v1.1** (paper v5, this release, 2026-06-02): per-layer ablation (L25/L26 alone, triple) + exp166 statistical confirmation + Goodhart's Law instance (triple) documentation
